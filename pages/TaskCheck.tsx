...
const TaskCheck: React.FC = () => {
  ...
  const handleCheck = async () => {
    ...
    let simulatedInputData = '';
    if (fileSelected.name.includes('fail')) {
        simulatedInputData = 'wrong_data';
    } else {
        simulatedInputData = 'demo_bypass'; 
    }

    const result = await Store.verifyTask(user.uid, task.id, simulatedInputData, task.isSpecial);
    ...
    if (result.success) {
        ...
        refreshUser();
        navigate('/tasks/' + (task.isSpecial ? 'special' : 'standard')); // Back to list
    } else {
    ...
    <Layout>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start space-x-3 mb-6">
          ...
          <p className="text-xs text-blue-800 leading-relaxed">
              ...
              <br/>For testing: Use any {task.isSpecial ? '.apk' : '.zip'} file. Rename to 'fail.zip' to test failure.
          </p>
      </div>
          ...
          <>
            ...
            <div className={`border-2 border-dashed rounded-2xl p-10 text-center transition-all relative overflow-hidden ${fileSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100'}`}>
                <input 
                  ...
                  title={task.isSpecial ? "Upload APK file" : "Upload ZIP file"}
                  aria-label={task.isSpecial ? "Upload APK file" : "Upload ZIP file"}
                />
